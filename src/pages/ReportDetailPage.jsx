...
import { ArrowLeft, CalendarDays, Check, CircleAlert, ClipboardList, ExternalLink, FilePenLine, Landmark, MapPin, MessageSquareText, Paperclip, Send, Users } from 'lucide-react'
    ...
    <div className="mx-auto max-w-[1400px] space-y-5 pb-10">
      <section className="rounded-xl border border-slate-800 bg-slate-900/75 p-5 sm:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          ...
          <div className="flex flex-wrap gap-2 xl:justify-end"><button onClick={() => navigate('/reports')} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-slate-700 px-3.5 text-sm font-semibold text-slate-300 hover:border-slate-600 hover:bg-slate-800"><ArrowLeft size={16} /> Danh sách</button>{canEdit && <button onClick={() => navigate(`/reports/${report.id}/edit`)} className="inline-flex min-h-10 items-center gap-2 rounded-lg border border-amber-400/30 bg-amber-400/10 px-3.5 text-sm font-semibold text-amber-200 hover:bg-amber-400/15"><FilePenLine size={16} /> Chỉnh sửa</button>}{canSubmit && <button disabled={isSubmitting} onClick={handleManualSubmit} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-cyan-400 px-3.5 text-sm font-bold text-slate-950 hover:bg-cyan-300 disabled:opacity-60"><Send size={16} /> {isSubmitting ? 'Đang nộp' : 'Nộp báo cáo'}</button>}{canReview && <><button onClick={() => { setReviewAction('reject'); setReviewModalOpen(true) }} className="min-h-10 rounded-lg border border-rose-400/30 bg-rose-400/10 px-3.5 text-sm font-semibold text-rose-200 hover:bg-rose-400/15">Yêu cầu chỉnh sửa</button><button onClick={() => { setReviewAction('approve'); setReviewModalOpen(true) }} className="inline-flex min-h-10 items-center gap-2 rounded-lg bg-emerald-400 px-3.5 text-sm font-bold text-slate-950 hover:bg-emerald-300"><Check size={16} /> Phê duyệt</button></>}</div>
        </div>
      </section>
