...
const roleLabels = {
  ...
  CLUB_MANAGER: 'Chủ câu lạc bộ',
  TREASURER: 'Thủ quỹ',
  CLUB_MEMBER: 'Tài khoản thành viên',
}
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="mt-1 text-sm text-gray-400">
          Vai trò hệ thống và tư cách CLB được cấp bởi quy trình xét duyệt, người dùng không thể tự thay đổi.
        </p>
      </motion.div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        >
          <h3 className="text-lg font-bold text-white">Thông tin xác thực</h3>
      ...
      <section className="rounded-2xl border border-slate-800 bg-slate-900/65 p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h3 className="text-lg font-bold text-white">Câu lạc bộ đã được phê duyệt</h3>
            <p className="mt-1 text-sm text-gray-400">Đơn đang chờ hoặc bị từ chối không xuất hiện trong danh sách quyền này.</p>
          </div>
        ...
        {approvedClubs.length === 0 ? (
          <div className="mt-5 rounded-xl border border-dashed border-slate-700 p-8 text-center text-gray-500">
            Tài khoản chưa thuộc câu lạc bộ nào.
          </div>
        ) : (
            ...
            {approvedClubs.map(access => (
              <div key={access.clubId} className="rounded-xl border border-slate-800 bg-slate-950/60 p-4">
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  {access.isManager && <span className="rounded-full bg-purple-500/15 px-2.5 py-1 text-purple-300">Chủ CLB</span>}
